...
def _resize_image_onnx(image, self_min_size, self_max_size):
    ...
    # image[None]操作是在最前面添加batch.维度[C,H,W]->[1,C,H,W]
    # bilinear只支持4D Tensor
    # 最后将4D的image 使用[0]就能实现降维的作用
...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def __init__(self, min_size, max_size, image_mean, image_std):
        ...
        # 这里判断 min_size 是不是list类型或者 tuple类型，如果不是就把 min_size 转换为tuple类型（其实这里我感觉没用）
    ...
    def resize(self, image, target):
        ...
        # 如果是训练模式，就不需要 同步缩放 bounding box的大小，直接输出预处理后的图像
        if target is None:
            return image, target

        bbox = target["boxes"]  # shape: [object_num, 4]
        # 根据图像的缩放比例来缩放bbox
    ...
    def batch_images(self, images, size_divisible=32):
        ...
        # 创建shape为batch_shape且值全部为0的tensor，相当于有一批 像素全为0的图片集合
    ...
    def postprocess(
        ...
        Args:
            result: list(dict), 网络的预测结果（包括候选框坐标以及类别信息）, len(result) == batch_size
...
def resize_boxes(boxes, original_size, new_size):
    ...
    # Removes a tensor dimension, boxes [minibatch, 4]  # shape: [object_num, 4]
    # Returns a tuple of all slices along a given dimension, already without it.
    xmin, ymin, xmax, ymax = boxes.unbind(1)  # unbind(1) 表示展开boxes的第一维数据，也就是展开 左上和右下坐标，注意它们都是列表类型
    xmin = xmin * ratios_width
    xmax = xmax * ratios_width
    ymin = ymin * ratios_height
    ymax = ymax * ratios_height
    # 使用stack在维度1上合并，就还原回去了
