...
class AnchorsGenerator(nn.Module):
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        Arguments:
            ...
            device: cpu/gpu
        """
        scales = torch.as_tensor(scales, dtype=dtype, device=device)  # 128, 256, 512
        aspect_ratios = torch.as_tensor(aspect_ratios, dtype=dtype, device=device)
        h_ratios = torch.sqrt(aspect_ratios)  # 1,  0.7,  1.414 (1, 0.5, 2)
        w_ratios = 1.0 / h_ratios  # 1, 1.428, 0.707

        # [r1, r2, r3]' * [s1, s2, s3]
        # number of elements is len(ratios)*len(scales)
        # ws 和 hs 就是按比例变化后的 图片的长宽
        ws = (w_ratios[:, None] * scales[None, :]).view(-1)  # (128, 1.428*128, 0.707*128......)
        hs = (h_ratios[:, None] * scales[None, :]).view(-1)  # (128, 0.7*128, 1.428*128......)
    ...
    def grid_anchors(self, grid_sizes, strides):
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            # 生成 0~grid_width（特征图的宽）的序列，这也就是特征图上的点，再乘上与原图像的步长，就能得到原图像上每一个点的坐标信息
